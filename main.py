...
#Windows Hardware/Routing/Networking/Troubleshooting v2.0
...
os.system('tracert 8.8.8.8 >> C:/test/test.txt')

#print("\nHere is the Path ping to Google.") #IF YOU WANT TO RUN THIS, USE Extended VERSION (Extended.py) #YOU CAN UNCOMMENT THESE LINES BUT UNDERSTAND THIS WILL INCREASE THE RUN TIME EXTREMELY LONG!
#os.system('echo "This is pathping, this tells you how many packets dropped along the route to google / 8.8.8.8 on each node. Takes a while like 4 minutes." >> C:/test/test.txt')
#os.system('pathping 8.8.8.8 >> C:/test/test.txt')

print("\nHere is the users to this node/system, MAKE SURE THERE IS NOT A USER YOUR NOT AWARE OF!")
os.system('echo "This lists all the users attached to this node" >> C:/test/test.txt')
os.system('net user >> C:/test/test.txt')
...
os.system('echo "Here is your ARP Table." >> C:/test/test.txt')
os.system('arp -a >> C:/test/test.txt')

print("\nHere is the all the open remote sessions connected to your node.")
os.system('echo "Here is the all the open network connections you have and the port #s:" >> C:/test/test.txt')
os.system('net session /list  >> C:/test/test.txt')
